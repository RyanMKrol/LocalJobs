...
import { api, type MovieRec } from '../lib/api';
 ...
 */
export function MovieRecsManager() {
  ...
  const ignore = (r: MovieRec) => item.run(r.tmdbId, () => api.ignoreMovieRec(r.tmdbId));
  const unignore = (r: MovieRec) => item.run(r.tmdbId, () => api.unignoreMovieRec(r.tmdbId));
  const unignoreAll = () => bulk.run(true, () => api.unignoreMovieRecBulk(ignored.map((r) => r.tmdbId)));
