 ...
 * API (T468 — split out of `movie-recommendations` into this dedicated
 * `missing-movies` workflow). For each owned movie with a tmdbId: GET
 * `/movie/{id}` → its `belongs_to_collection`. For each DISTINCT collection:
 * GET `/collection/{id}` → `parts[]`; a gap is any RELEASED part whose tmdb id
 * is NOT owned. Dedupes collection fetches (each fetched once even if several
 * members are owned). NO quality filter, NO skip heuristics — every factual gap
 * is surfaced; the TMDB rating rides along for context only. TMDB calls route
 * through the shared rate-limited `tmdb` service. RE-COMPUTES FRESH every run.
 *
 * The pure detection helpers (`buildOwnedSet`/`collectionGaps`/
 * `collectionOwnedExample`) are reused from `../../movies/movies.js` rather
 * than duplicated — mirroring `movies.ts`'s own existing precedent of reusing
 * `extractTmdbId` from `../missing-tv-seasons/plex.js` across workflow folders.
 */
export async function runFranchiseGaps(ctx: JobContext, opts: FranchiseGapsOpts = {}): Promise<void> {
  ensureDirs();
  const now = opts.now ?? new Date();
  const snapshotFile = opts.snapshotFile ?? missingMoviesConfig.snapshotOut;
  ...
  if (!existsSync(snapshotFile)) {
    throw new Error(`snapshot.json not found — run plex-movie-snapshot first (${snapshotFile}).`);
  }
