...
const job: JobDefinition = {
  ...
  description: 'First stage of the weekly missing-movies franchise-gap audit. Reads the owner\'s Plex movie library section (PLEX_MOVIE_SECTION) via the shared Plex client, matching each movie to its TMDB id from the tmdb:// GUID Plex records — a movie with no such GUID is flagged and excluded from downstream franchise-gap checking. Writes snapshot.json (every owned movie plus its TMDB id, used as the owned-set for franchise-gap detection). This is a DELIBERATE duplicate of movie-recommendations\'s own movie-snapshot job (own job, own data/out/) rather than a shared stage, mirroring missing-tv-seasons\'s plex-tv-snapshot vs tv-recommendations\'s tv-snapshot; it also skips building a taste profile since franchise-gaps never reads one. This stage recomputes fresh every run rather than skipping already-seen movies, since the Plex library itself can change week to week.',
  timeoutMs: 300_000,
  maxRetries: 3,
