 ...
 *  Unchanged from the pre-split combined workflow — no work_items migration needed. */
...
export async function runNotify(ctx: JobContext, opts: NotifyOpts = {}): Promise<void> {
  ...
  // Send the digest, then record each notified gap so it never repeats.
