...
/**
 * The work_items key-space for the franchise-gap "already-notified / ignored"
 * ledger. UNCHANGED from before the T468 split (when this job lived in
 * `movie-recommendations` as part of a combined gaps+recs digest) — this is
 * the SAME job name and ledger key as before, so every existing ignored/
 * notified row keeps resolving with zero data migration.
 *
 * `src/api/server.ts`'s dashboard endpoints (out of scope for the T468 build
 * task) still import `NOTIFY_JOB`/`gapKey` from
 * `../workflows/movies/stages/notify.js`, which keeps its own compatibility
 * copy of these same two constants (identical values) purely so that import
 * path keeps resolving — see the comment at the top of that file. This file's
 * copy is the source of truth going forward for the actual gaps notify job.
 */
...
export function buildDigest(gaps: FranchiseGap[]): { count: number; title: string; body: string } {
  const g = gaps.length;
  const names = gaps.map((x) => x.title).slice(0, 10);
  const body = names.join(', ') + (gaps.length > names.length ? `, +${gaps.length - names.length} more` : '');
...
/**
 * Terminal stage — the weekly franchise-gaps-only digest (T468 split; this used
 * to also cover taste recommendations as part of `movie-recommendations`,
 * now `movie-recommendations`'s own `movie-recs-notify`). Reads the fresh
 * franchise gaps, drops anything the owner has manually IGNORED, finds the
 * ones NOT already in the "notified" ledger, sends one digest, then marks each
 * notified gap done so it never repeats. Also (re)writes a weekly markdown
 * report grouped by collection.
 *
 * The ledger (`NOTIFY_JOB`, keyed by the missing film's tmdb id) is a
 * "notified" log, NOT a work-done log — a gap is announced at most once ever,
 * and an `ignored` gap leaves BOTH the report AND notifications.
 */
...
/**
 * Write the weekly markdown report: franchise gaps grouped by collection.
 * Returns its absolute path. Newly-detected items are flagged 🆕.
 */
