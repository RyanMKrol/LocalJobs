// Notify-stage tests (T468 split — gaps-only; the franchise-gaps section of the
// old combined src/workflows/movies/stages/notify.test.ts, moved here) — dedup
// + digest + ignore-to-suppress against the work_items ledger. Hermetic: NO
// live push (an injected capture fn), synthetic franchise-gaps file, scratch
// DB (npm test points LOCALJOBS_DB at /tmp). Covers: first run = one digest of
// the whole backlog; an already-notified gap is NOT re-notified; a freshly
// detected gap aggregates into a new single digest; an owner-IGNORED gap is
// excluded from BOTH the report AND notifications; ignoring an
// already-notified gap also removes it from future reports; a failed push
...
// Run 1 — first run sends ONE digest of the whole backlog and marks the ledger.
{
  ...
  assert.match(md, /\[Saw X\]\(https:\/\/www\.themoviedb\.org\/movie\/9990110\)/);
...
{
  const sent: CapturedPush[] = [];
  const grown = [...backlog, { collectionId: 9, collectionName: 'Jurassic Park Collection', tmdbId: 9990113, title: 'Jurassic World Rebirth', year: 2025, tmdbRating: 6.8 }];
  writeGaps(grown);
  await runNotify(fakeCtx(), { push: capturePush(sent), now: NOW, gapsFile, reportDir });
...
// it's notified: it must be excluded from BOTH the digest AND the report.
{
  ...
  const withIgnored = [
    ...backlog,
    { collectionId: 9, collectionName: 'Jurassic Park Collection', tmdbId: 9990113, title: 'Jurassic World Rebirth', year: 2025, tmdbRating: 6.8 },
