// Typed-artifact contracts for the missing-movies (franchise-gap audit, T468
// split) stage boundaries.
//
//   plex-movie-snapshot ──plex-movie-snapshot──▶ franchise-gaps ──franchise-gaps──▶ movie-gaps-notify
//
// Mirrors the shape/factory pattern of the sibling `movie-recommendations`
// workflow's `contracts.ts` (its `movieSnapshotContract`/`franchiseGapsContract`
// were the template) — deliberately its OWN copy rather than a shared import,
// since this workflow has its OWN snapshot/gaps artifacts on disk (T467 design
// decision: duplicate, not share).
//
// Each factory returns an ArtifactContract whose `check()` inspects the REAL
// JSON artifact left on disk and reports SHAPE + NON-EMPTY drift — enough to
// catch a Plex/TMDB response-shape change or an empty hand-off without brittle
// full-schema validation. They take an optional path so unit tests can point
// at fixtures.
...
export function missingMoviesSnapshotContract(file: string = missingMoviesConfig.snapshotOut): ArtifactContract {
  return {
    key: 'plex-movie-snapshot',
    description: 'snapshot output: { movies: [{ title, tmdbId, ratingKey }] } — readable, with GUID-matched movies.',
    shape: {
      summary: 'The fresh Plex movie snapshot: each movie with its TMDB id (no taste metadata — this workflow never needs it).',
