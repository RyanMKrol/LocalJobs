...
**This workflow is RECS-ONLY (T468).** The deterministic franchise-gap audit that used to share this
DAG has moved to its own workflow — see `src/workflows/missing-movies/CLAUDE.md` — mirroring the
existing `tv-recommendations` / `missing-tv-seasons` split. The `franchise-gaps` job name and the
`movie-gaps-notify` job name/ledger key are UNCHANGED from before the split (no `work_items`
migration was needed); they just live under `missing-movies/` now.

## What it does

Monthly taste-based movie RECOMMENDATIONS from the owner's Plex library: 8 parallel Claude
recommender branches propose picks from a stratified library sample, merged with code-side TMDB
verification, cross-branch dedup, and per-genre balancing, pushed as ONE monthly digest. Dedupes per
TMDB id so nothing repeats across runs; the owner can ignore-to-suppress a recommendation from the
dashboard.
...
this?" / "has the owner ignored it?" log, not a work-done ledger — so a backlog recommendation is
announced exactly once and an ignored one is suppressed forever.
...
movie with no `tmdb://` GUID is flagged and excluded, listed in the run's logs). Writes:
...
This is this workflow's OWN snapshot, separate from `missing-movies`'s `plex-movie-snapshot` (same
Plex section, two independent jobs/files — mirrors `tv-recommendations`'s snapshot vs
`missing-tv-seasons`'s `plex-tv-snapshot`).
...
a monthly push covering only recommendations not yet notified (`work_items` ledger, keyed
`recKey(tmdbId)`). Writes `data/out/reports/recommendations.md`. The first run digests the whole
current backlog; later runs only cover what's new since the last one. Also appends newly-notified
recommendations to `data/out/recs-history.json`.

## Ignore-to-suppress (owner UI)

- `POST /api/movie-recs/:tmdbId/ignore` / `/unignore`, plus bulk `unignore-bulk` — backed by
  `ignoreSurfacedItem(RECS_JOB, recKey(tmdbId))` / `unignoreSurfacedItem` — upserts the ledger row to
  `ignored` even if none exists yet. An ignored rec is excluded from BOTH the merge stage (never
  re-suggested) and future digests.
...
- The equivalent franchise-gap ignore-to-suppress (`/api/movie-gaps/*`) now belongs conceptually to
  `missing-movies` (see that workflow's `CLAUDE.md`) — its management UI still renders on THIS
  workflow's detail page for now (dashboard relocation is the queued T469 follow-up, out of T468's
  scope).

## Files, credentials, config

- `config.ts` (`moviesConfig`) — `data/out/` paths for the snapshot/recs artifacts, the 10+
  `MOVIES_RECS_*` tuning env vars, `PLEX_MOVIE_SECTION`. `gapsOut` is a **compat shim (T468)**:
  the franchise-gaps audit output moved to `missing-movies`'s own `data/out/`, but
  `src/api/server.ts`'s `/api/movie-gaps` endpoints still read `moviesConfig.gapsOut` directly —
  it's aliased to `missingMoviesConfig.gapsOut` so that keeps resolving without touching
  `server.ts` (out of scope for T468; a follow-up task relocates that import properly).
- `contracts.ts` — gate contracts for every DAG edge in THIS workflow (snapshot→each branch,
  each branch→merge, merge→notify). The franchise-gaps contract moved to
  `missing-movies/contracts.ts`.
...
- `movies.ts` — pure snapshot/collection-gap logic (`buildMovieSnapshots`, `buildOwnedSet`,
  `buildTasteProfile`, `collectionGaps`, `collectionOwnedExample`). Kept here (not moved) because
  `buildMovieSnapshots`/`buildOwnedSet` are still used by THIS workflow's own snapshot;
  `missing-movies`'s stages import `collectionGaps`/`collectionOwnedExample`/`buildOwnedSet` from
  here too, rather than duplicating the pure math (the same cross-workflow pure-helper-reuse
  convention `movies.ts` itself uses for `extractTmdbId` from `missing-tv-seasons/plex.ts`).
