...
// run = one digest of the whole backlog; an already-notified rec is NOT re-notified;
// a freshly detected rec aggregates into a new single digest; an owner-IGNORED rec is
// excluded from BOTH the report AND notifications; history is appended on notify.
//
// The franchise-gaps half of this test suite moved to
// src/workflows/missing-movies/stages/notify.test.ts along with the gap audit itself
// (T468) — `buildDigest`/`runNotify` here are now recs-only.
...
// ── buildDigest aggregates count + caps the body, recs-only ──
{
...
// Run R1 — first recs run sends ONE digest, writes the report, marks the ledger, and
// appends history.
{
  ...
  console.log('  ✓ recs: first run digests, writes the report, marks ledger + history');
}
...
// Run F1 — a FAILED digest push must not poison the ledger: `runNotify` throws, no
// ledger row is written for the new rec, and history is not appended, so the next
// run retries fresh.
{
...
console.log('  ✓ movies notify (recs-only) dedup/digest/ignore tests passed');
