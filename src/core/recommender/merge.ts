// Generic recommender merge stage (T561) — shared by every domain. Pools all
// branches' raw suggestions, verifies+dedupes+quality-filters them into the
// final balanced list, and runs a bounded top-up loop when short of target.
// A domain supplies the TMDB search/genre mapping + config/paths; this module
// owns the verify/dedup/balance/top-up mechanics (byte-identical across
// movies/tv-recs before this extraction).
...
export type SearchFn = (title: string, year: number | null) => Promise<TmdbSearchMatch | null>;
...
export interface MergeRunOpts {
  search?: SearchFn;
  /** Inject the top-up source (tests). Defaults to fanning out over the domain's branches. */
  topUp?: TopUpFn;
  ...
  recsOut?: string;
  now?: Date;
  // ── Threshold/target overrides (default from domain.config; tests pass these) ──
  minRating?: number;
  minVotes?: number;
  target?: number;
  genreCap?: number;
 ...
 * NOT already recommended/ignored → clear the QUALITY bar (TMDB rating ≥
 * minRating AND vote_count ≥ minVotes). Dedupes across branches by resolved
 * tmdb id (merging lenses). Mutates `byTmdb` + `counters`; stops early (setting
 * `counters.quotaHit`) when TMDB's quota is reached.
 */
async function verifyInto<M, P>(
  batch: RawSuggestion[],
  ...
  for (const s of batch) {
    let result: TmdbSearchMatch | null;
    try {
    ...
    if (isWorkItemDone(domain.recsJob, recKey(tmdbId), 1)) {
    ...
    byTmdb.set(tmdbId, {
      ...
      reason: s.reason,
      lens: s.lens,
      genre: domain.genreName(result.genre_ids),
      tmdbRating: rating,
    });
  }
}
...
/**
 * The default top-up source: fan out over all of the domain's branch specs
 * IN-MEMORY (no file I/O), re-prompting each Claude branch for ADDITIONAL
 * well-regarded items while excluding everything already collected/considered
 * this run. Built lazily so a run that never goes under target loads no taste
 * profile.
 */
function buildDefaultTopUp<M, P>(
  domain: RecommenderDomain<M, P>,
  snapshot: unknown,
  tasteFile: string,
  historyFile: string,
  ...
  return async (exclude, round) => {
    ...
    ctx.log(`  Re-prompting ${domain.branches.length} branch(es) (round ${round}, concurrency ${limit}), excluding ${exclude.length} already-collected/owned/considered title(s); ${alreadySuggested.length} already-suggested history title(s) in context…`);
    const pooled: RawSuggestion[] = [];
    await runBounded(domain.branches, limit, async (spec) => {
      let more: RawSuggestion[];
      try {
        more = await collectBranchSuggestions(
          spec,
          { profile, items, recent, alreadySuggested, sampleSize: domain.config.recsSampleSize, ask: domain.config.recsPerBranchAsk, exclude },
          run,
          domain.config.recsModel,
        );
      } catch (err) {
 ...
 * runs a BOUNDED top-up loop: re-prompt the branches for more, verify + merge,
 * repeat up to `topUpRounds` rounds or until the target is reached / no new
 * suggestions arrive. Writes recommendations.json. Per-item search failures are
 * skipped (continue to the next suggestion) and a TMDB quota stops verification
 * gracefully — the list is still always written. But if ANY suggestion's TMDB
 * search errored this run, the stage then THROWS a summarizing error, failing
 * the run so it's retried.
 */
export async function runMerge<M, P>(
  ctx: JobContext,
  domain: RecommenderDomain<M, P>,
  opts: MergeRunOpts = {},
): Promise<void> {
  const search = opts.search ?? ((title: string, year: number | null) => domain.search(title, year));
  ...
  const recsOut = opts.recsOut ?? domain.config.recsOut;
  const now = opts.now ?? new Date();
  const params: MergeParams = {
    minRating: opts.minRating ?? domain.config.recsMinRating,
    minVotes: opts.minVotes ?? domain.config.recsMinVotes,
    target: opts.target ?? domain.config.recsTarget,
    genreCap: opts.genreCap ?? domain.config.recsGenreCap,
    topUpRounds: opts.topUpRounds ?? domain.config.recsTopUpRounds,
  };

  ctx.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  ctx.log(`${domain.mergeStageName} starting`);
  if (!existsSync(snapshotFile)) {
    throw new Error(`snapshot.json not found — run ${domain.snapshotStageName} first (${snapshotFile}).`);
  }
  const snapshot = JSON.parse(readFileSync(snapshotFile, 'utf8'));
  const owned = buildOwnedSet(domain.itemsOf(snapshot) as unknown as { tmdbId: number | null }[]);
  ...
  // Also exclude currently owner-ignored recommendations from the top-up prompt,
  // even if they were never appended to the history file (a pre-notify ignore).
  for (const title of ignoredSuggestionTitles(domain.recsJob)) {
    if (!considered.includes(title)) considered.push(title);
  }

  ctx.progress(20, `verifying ${unique.length}`);
  await verifyInto(unique, byTmdb, owned, domain, search, ctx, params, counters);

  let balanced = balanceByGenre([...byTmdb.values()], { cap: params.genreCap, target: params.target });

  // ── Bounded top-up loop: re-prompt the branches until we hit the target. ──
  const topUp = opts.topUp
    ?? buildDefaultTopUp(domain, snapshot, tasteFile, historyFile, opts.runClaude ?? runClaude, ctx, opts.topUpConcurrency ?? domain.config.recsTopUpConcurrency);
  let round = 0;
  let topUpStopReason = '';
  while (!counters.quotaHit && balanced.length < params.target && round < params.topUpRounds) {
    ...
    await verifyInto(fresh, byTmdb, owned, domain, search, ctx, params, counters);
  ...
  ctx.log('');
  ctx.log(`═══════════════ ${domain.mergeStageName.toUpperCase()} SUMMARY ═══════════════`);
