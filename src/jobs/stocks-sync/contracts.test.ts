...
test('both contracts declare a shape with a summary + non-empty expectations', () => {
  for (const c of [stocksPortfolioContract(), stocksFreshBreachesContract()]) {
...
test('the stocks-sync DAG derives exactly 2 gates matching its stage boundaries', () => {
  const dag = buildDag([
    { job: 'stocks-snapshot' },
  ...
  assert.equal(gates.length, 2, JSON.stringify(gates));
  assert.ok(
