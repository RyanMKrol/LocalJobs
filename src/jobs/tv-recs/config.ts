...
/**
 * Connectivity + paths for the Plex TV recommendations workflow. Shares Plex host
 * + token and TMDB token with the existing plex/missing-tv-seasons workflow via
 * `plexConfig` — only the TV section and data paths are tv-recs-specific.
 */
export const tvRecsConfig = {
  ...
  // ── Shared connectivity (reused from the TV plex workflow) ──
